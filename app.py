    ...
    if request.method == 'POST':
        ...
        if session['add_flag'] == "delete":
            ...
            if session['add_flag'] == "add":
                insert_query = "insert into [DataMining_A1].[dbo].[MobileInfo] (Rank, Model, Specs, Price, Rating, Votes, Image) values ({0},\'{1}\',\'{2}\',\'{3}\',\'{4}\',\'{5}\',\'{6}\');".format(rank, model, specs, price, rating, votes, image)
                session.pop('add_flag', None)
            elif session['add_flag'] == "edit" :
                insert_query = "update [DataMining_A1].[dbo].[MobileInfo] set Model=\'{0}\', Specs=\'{1}\', Price=\'{2}\', Rating=\'{3}\', Votes=\'{4}\',Image=\'{5}\' where Rank={6};".format(model, specs, price, rating, votes, image, rank)
                session.pop('add_flag', None)
...
def edit_data(id):
    ...
    return render_template('edit.html', out = [e_out.to_html(classes='data', header = True, index=False)], ran = id, mod=e_out.iloc[0]['Model'], spe=e_out.iloc[0]['Specs'], pri=e_out.iloc[0]['Price'], rat=e_out.iloc[0]['Rating'], vot=e_out.iloc[0]['Votes'], img=e_out.iloc[0]['Image'])
